...
describe('NASA proxy handlers', () => {
  ...
  beforeEach(() => {
    store = createStore()
    setStoreRef(store)
    // Set nasa as active app so tool gating allows NASA tools
    store.set(activeAppAtom, 'nasa')
    setBridgeRef({
...
describe('getChatBridgeTools includes NASA tools', () => {
  it('does NOT include NASA tools when no app is active (tool gating)', () => {
    const tools = getChatBridgeTools(null)
    const names = tools.map((t) => t.name)
    expect(names).not.toContain('get_apod')
